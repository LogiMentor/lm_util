...
entity tb_vu_lm_util_crc_serial is
  generic (
    -- CRC-16/GENIBUS
...
begin
  ...
  begin
    ...
    info("v_lsb=" & f_slv2string(f_string2slv_lsb(g_test_str)));
    info("v_msb=" & f_slv2string(f_string2slv(g_test_str)));
    ...
    if run("serial") then
      ...
      -- clock-in data
      dv_i <= '1';
      for i in C_TOTAL_BITS - 1 downto 0 loop
        data_i <= v_msg(i);
        p_wait_clk(clk_i);
      end loop;
      wait for 1 ps;
